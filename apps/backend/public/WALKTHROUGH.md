...
- [Spice.Watch main] More ways to watch: VidLink and MoviesAPI join VidSrc as switchable in-player sources (choice remembered), so a bad dub on one backend is one tap away from another.
- [Spice.Watch main] My List grows up: Watch Later / Watching / Completed / Dropped shelves (progress auto-files — starting moves titles to Watching, finishing to Completed), a release bell for list titles that dropped in the past 30 days, and a Coming soon / Airing now timeline with real dates. Same tables and endpoints the anime surface will reuse.
- [Spice.Movies main] Fancy frame: a shared sidebar + topbar with a profile menu (sign-in, account email, default watch source, sign-out) across movies and shows, a self-scrolling trending billboard with Play and My List on both homes, and a TV series spotlight on the movies landing. All pictograms are a shared SVG set, movies/shows wear their own clapperboard favicon instead of the music note, and the profile menu shows your synced music PFP (avatar included, initial-letter fallback).
